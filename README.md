...
A local-first macOS desktop app for tracking IT incidents, calculating operational metrics, and generating polished DOCX reports for quarterly and annual leadership reviews.
...
- Bulk status updates with multi-select
...
- One-click quarterly or annual report generation
...
- Auto-generated discussion points based on 10 data-driven rules (e.g., "VPN had 3+ incidents — systemic improvement needed?")
- Dashboard charts embedded as PNG images
- Custom title and introduction text
...
- Service registry with 15 pre-configured services
- Fiscal quarter configuration (FY27 Q1-Q4 seeded)
- 154 security and correctness tests
...
```
src/                          # React frontend
  components/                 # UI components (incidents, dashboard, reports, import, settings)
...
src-tauri/src/                # Rust backend
  ...
  db/                         # SQLite initialization, migrations, query modules
  models/                     # Data structs, validation, priority matrix
  reports/                    # DOCX generation with 8 report sections
  import/                     # CSV parsing and column mapping
  security_tests.rs           # 154 security and correctness tests
```
...
**Tables:** `incidents`, `services`, `action_items`, `quarter_config`, `import_templates`, `app_settings`

15 services and FY27 Q1-Q4 quarters are seeded on first run.
